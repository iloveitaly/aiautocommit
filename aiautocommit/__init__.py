...
_TRAILER_LINE = re.compile(r"^[0-9A-Za-z][0-9A-Za-z-]*[:=](\s.*)?$")
...
def is_git_trailer(line: str) -> bool:
...
def generate_commit_message(diff):
    ...
    if message.strip().startswith("#"):
        return message
    return apply_commit_suffix(message, COMMIT_SUFFIX)
...
def check_lock_files():
    ...
    if len(unique_messages) == 1:
        return apply_commit_suffix(unique_messages.pop(), COMMIT_SUFFIX)

    return apply_commit_suffix("chore(deps): update lock files", COMMIT_SUFFIX)
    ...
    with log_execution_time("overall_execution"):
        ...
        try:
            ...
            if not staged_diff:
                ...
                diff = get_diff()
                if not diff:
                    commit_message = apply_commit_suffix(
                        "style: whitespace change", COMMIT_SUFFIX
                    )
                else:
                    try:
