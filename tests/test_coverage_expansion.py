...
from aiautocommit import main, is_reversion, check_lock_files, LOCK_FILE_MESSAGES, update_env_variables, configure_prompts
...
def test_check_lock_files_exhaustive(git_repo):
    for lock_file, expected_msg in LOCK_FILE_MESSAGES.items():
        subprocess.run(["git", "rm", "-rf", "--cached", "."], stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
...
def test_get_difftastic_diff_success():
    with patch('shutil.which', return_value='/usr/local/bin/difft'):
        ...
        with patch('aiautocommit.difftastic.run_command', return_value=mock_result) as mock_run:
...
def test_commit_no_internet(runner, git_repo):
    ...
    with patch('aiautocommit.get_diff', return_value="some diff"):
        with patch('aiautocommit.wait_for_internet_connection', side_effect=Exception("No internet")):
...
def test_commit_binary_file_exit_path(runner, git_repo):
    with patch('aiautocommit.get_diff', side_effect=UnicodeDecodeError("codec", b"", 0, 1, "reason")):
        result = runner.invoke(main, ["commit"])
        assert result.exit_code == 1
